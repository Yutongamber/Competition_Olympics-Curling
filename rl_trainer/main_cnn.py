...
parser.add_argument('--opponent', default="random",
                    help="random or run1")  # NOTE:默认值是random
...
parser.add_argument("--load_model", action='store_true')  # 命令行加是true；不加为false
parser.add_argument("--load_run", default=7, type=int)
parser.add_argument("--load_episode", default=2800, type=int)
...
parser.add_argument("--use_OpponentPool", action='store_true', default=False,
                    help="use opponent pool")
...
def main(args):
    ...
    if not args.load_model: #NOTE: if not args.
    ...
    while episode < args.max_episodes:
        ...
        temp=np.array(state[ctrl_agent_index]['obs'])
        
        episode += 1
        step = 0
        Gt = 0
        flag=0 # 0表示是先手，1表示是后手
        save_ego_ball=10
        OnlyRunOnce_First=1 # 确保后面的if程序只运行一次
        OnlyRunOnce_Mediate=1
        ...
        while True:
            ...
            EgoAgent_ThrowLeft=all_observes[ctrl_agent_index]['throws left'][ctrl_agent_index]
            OppoAgent_ThrowLeft=all_observes[ctrl_agent_index]['throws left'][1-ctrl_agent_index]
             ############################用于判断先手还是后手########################################
            if (EgoAgent_ThrowLeft<OppoAgent_ThrowLeft) and OnlyRunOnce_First==1:
                flag=0 # ego agent是先手
                OnlyRunOnce_First=0
            elif OnlyRunOnce_First==1 and (EgoAgent_ThrowLeft>OppoAgent_ThrowLeft): #NOTE 下午验证一下
                flag=1
                OnlyRunOnce_First=0
                
            if ((save_ego_ball-all_observes[ctrl_agent_index]['throws left'][ctrl_agent_index])==-3) and OnlyRunOnce_Mediate==1:
                # print('对局切换')
                OnlyRunOnce_Mediate=0
                if flag==1:
                    flag=0
                else:
                    flag=1
            ...
            ############################ 对手池 and collect opponent action #################################
            if args.use_OpponentPool:
            ...
            all_observes=env.all_observes # 获取前一时刻的状态
            save_ego_ball=all_observes[ctrl_agent_index]['throws left'][ctrl_agent_index]
            ...
            if flag==0: # ego agent是先手
                next_obs_ctrl_agent=np.vstack((first_hand_array,next_obs_ctrl_agent))
            else: 
                next_obs_ctrl_agent=np.vstack((back_hand_array,next_obs_ctrl_agent))
            step += 1
            ...
            temp=np.array(next_state[ctrl_agent_index]['obs'])
            ...
            if done:
                ...
                break
        if episode % args.save_interval == 0 and not args.load_model: #NOTE:  not args.
